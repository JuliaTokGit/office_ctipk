...
var Range = Element.extend(vertical).extend({
  initialize: function initialize(options) {
    ...
    if(typeof this.options.rangeClass === 'function') {
      this.writerangeClass(self.options.rangeClass.bind(self));
    } else {
      this.writerangeClass(this.options.rangeClass);
    }
  ...
  mousedown: function(ev) {
    ...
    } else if (ev.which >= 2 && this.perant.options.allowDelete) {
        if(this.deleteConfirm) {
          this.perant.removeRange(this);
          clearTimeout(this.deleteTimeout);
        } else {
...
var RangeBar = Element.extend(vertical).extend({
  ...
  addRange: function(range, data) {
    // console.log(data);
    var readonly=false;
    if (typeof data != 'undefined'){ readonly=data.readonly; }
    var $range = Range({
      perant: this,
