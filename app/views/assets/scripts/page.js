...
$('#addview').on('click',function(e){
  e.preventDefault();
  $('#addview-container').appendTo('#rubikon').removeClass('hidden');
  $('#addview').hide();
});

$('#addjs').on('click',function(e){
  e.preventDefault();
  $('#addjs-container').appendTo('#rubikon').removeClass('hidden');
  $('#addjs').hide();
});
  ...
  if ($('#json_editor').length){
    ...
    $.getJSON('/schemas/id/page', function (schema){
      ...
      editor.on('ready',function() {
        ...
        $('.json-editor-btn-submit').on('click',function() {
          ...
          if(errors.length) {
            ...
            $.ajax({
              method:"POST",
              url:'/config/collection/pages/item/'+id,
              dataType:'json',
